...
function _linkColor(Inputs, URLSearchParams, html) {
  return Inputs.select(
    ...
    {
      value:
        new URLSearchParams(html`<a href></a>`.search).get("color") ||
        "source-target",
      label: "Link color",
    }
  );
...
function _nodeAlign(Inputs) {
  return Inputs.select(["left", "right", "center", "justify"], {
    value: "justify",
    label: "Node alignment",
  });
}

function _chart(SankeyChart, energy, nodeAlign, linkColor, d3, width) {
  return SankeyChart(
    {
      links: energy,
    },
...
function _energy(FileAttachment) {
  return FileAttachment("energy.csv").csv({ typed: true });
}

function _6(howto) {
  return howto("SankeyChart", { imports: { d3: "d3", d3Sankey: "d3-sankey" } });
}

function _SankeyChart(d3Sankey, d3) {
  return function SankeyChart(
    {
...
export default function define(runtime, observer) {
  ...
  main
    .variable(observer("linkColor"))
    .define("linkColor", ["Generators", "viewof linkColor"], (G, _) =>
      G.input(_)
    );
  main
    .variable(observer("viewof nodeAlign"))
    .define("viewof nodeAlign", ["Inputs"], _nodeAlign);
  main
    .variable(observer("nodeAlign"))
    .define("nodeAlign", ["Generators", "viewof nodeAlign"], (G, _) =>
      G.input(_)
    );
  main
    ...
    .define(
      "chart",
      ["SankeyChart", "energy", "nodeAlign", "linkColor", "d3", "width"],
      _chart
    );
  main
    .variable(observer("energy"))
    .define("energy", ["FileAttachment"], _energy);
  main.variable(observer()).define(["howto"], _6);
  main
    .variable(observer("SankeyChart"))
    .define("SankeyChart", ["d3Sankey", "d3"], _SankeyChart);
  main.variable(observer()).define(["md"], _8);
  main
    .variable(observer("d3Sankey"))
    .define("d3Sankey", ["require", "d3"], _d3Sankey);
